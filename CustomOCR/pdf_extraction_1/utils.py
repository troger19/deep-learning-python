...
def extract_dynamic_fields(page, phrases):
    ...
    for i, phrase in enumerate(phrases):
        ...
        if target_word is not None:
            ...
            dynamic_fields.update({phrase: target_word})
...
def find_final_value(row, template_type):
    if template_type =='decimal':
        return re.search('\d+[.,]\d+', str(row)).group()
...
def extract_correct_row(extracted_dynamic_fields, phrase):
    ...
    for top in top_values:
        ...
        pixels_from_top_range = np.arange(top - 7, top + 7, ).tolist()
